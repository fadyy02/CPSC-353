...
int main(int argc, char* argv[]) {
    ...
    mode = atoi(argv[1]); // 1: encrypt, 2: decrypt
    string cipher = argv[2]; // caesar, vigenère, otp
	input = argv[3]; // text to be encrypted/decrypted

	toupper(cipher[0]); // check
	toupper(input[0]); // check
    ...
    if (argc != 4 && argc != 5) { // Check for the correct number of arguments
        ...
        cout << "Ciphers: caesar, Vigenère, otp" << endl;
